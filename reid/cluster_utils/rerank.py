...
def re_ranking(original_dist, k1=20, k2=6, lambda_value=0.3):
    ...
    for i in range(all_num):
        ...
        # k1+1 because self always ranks first. forward_k_neigh_index.shape=[k1+1].  forward_k_neigh_index[0] == i.
        forward_k_neigh_index = initial_rank[i, :k1+1]
        # backward.shape = [k1+1, k1+1]. For each ele in forward_k_neigh_index, find its rank k1 neighbors
        backward_k_neigh_index = initial_rank[forward_k_neigh_index, :k1+1]
        fi = np.where(backward_k_neigh_index == i)[0]
        # get R(p,k) in the paper
